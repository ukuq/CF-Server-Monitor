...
| 合并任务 | `0 * * * *`   | 每小时执行，根据日期判断执行：每周日 00:00 UTC 数据轮换、每天 12:00 UTC 服务器到期检测 |
...
写入行：默认 60 秒上报时，1 台服务器一天约占用 1.44k 写入行。免费写入额度是 100k/天，理论上可支持约 69 台服务器监控；如果修改上报频率为 120 秒可用翻倍。
